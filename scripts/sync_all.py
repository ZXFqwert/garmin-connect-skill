...
def init_db():
    ...
    for col_def in new_daily:
        ...
        try:
            ...
            print(f"  + 新增 daily_metrics.{col_name}")
        except:
            pass
    ...
    for col_def in new_sleep:
        col_name = col_def.split()[0]
        try:
            c.execute(f"ALTER TABLE sleep_data ADD COLUMN {col_def}")
            print(f"  + 新增 sleep_data.{col_name}")
        except:
            pass
...
def write_sleep_data(date: str, data: dict, sync_time: str):
    ...
    c.execute("""
        INSERT OR REPLACE INTO sleep_data (
            ...
            avg_heart_rate, avg_spo2, last_sync,
...
def fetch_day_data(garmin_client, date_str: str) -> Optional[dict]:
    ...
    try:
        stress_resp = garmin_client.getStressData(date_str) or {}
        if isinstance(stress_resp, dict):
            daily = stress_resp.get('dailyStressLevel') or {}
            stress = {
                'average': safe_int(daily.get('averageStressLevel')),
                'max': safe_int(daily.get('maxStressLevel')),
            }
    except Exception as e:
        print(f"    ⚠️ getStressData: {e}")
    ...
    return {
        ...
        'summary': {
            ...
            'moderate_intensity_minutes': safe_int(isinstance(stats, dict) and stats.get('moderateIntensityMinutes')),
            'vigorous_intensity_minutes': safe_int(isinstance(stats, dict) and stats.get('vigorousIntensityMinutes')),
            'calories_active': safe_float(isinstance(stats, dict) and stats.get('activeKilocalories')),
            'calories_bmr': safe_float(isinstance(stats, dict) and stats.get('bmrKilocalories')),
            'floors_descended': safe_float(isinstance(stats, dict) and stats.get('floorsDescended')),
            'intensity_minutes': safe_int(isinstance(stats, dict) and (stats.get('moderateIntensityMinutes', 0) + stats.get('vigorousIntensityMinutes', 0))),
        },
        'sleep': sleep,
        'workouts': activities,
        'body_battery': bb,
