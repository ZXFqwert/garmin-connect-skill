...
def generate_morning_report(data):
    ...
    has_main_sleep = sleep.get('duration_hours', 0) > 0
    ...
    if body_battery.get('current', 0) > 0:
        lines.append("🔋 身体电量")
        lines.append(f"  • 当前：{body_battery['current']}%")
        lines.append(f"  • 充电/消耗：+{body_battery['charged']}% / -{body_battery['drained']}%")

        if body_battery['current'] >= 70:
            lines.append("  • 评价：✅ 精力充沛")
        elif body_battery['current'] >= 40:
    ...
    if stress.get('average', 0) > 0:
        lines.append("😌 压力指数")
        lines.append(f"  • 平均压力：{stress['average']}/100")
        lines.append(f"  • 休息状态：{stress['rest_percentage']:.1f}%")

        if stress['average'] <= 25:
            lines.append("  • 评价：✅ 压力很低，状态很好")
        elif stress['average'] <= 50:
    ...
    if summary.get('heart_rate_min', 0) > 0:
        lines.append(f"  • 最低心率：{summary['heart_rate_min']} bpm")
    if summary.get('heart_rate_max', 0) > 0:
        lines.append(f"  • 最高心率：{summary['heart_rate_max']} bpm")
    ...
    if summary.get('floors_ascended', 0) > 0:
        lines.append(f"  • 爬楼层数：{summary['floors_ascended']:.1f} 层")

    if summary.get('calories', 0) > 0:
        lines.append(f"  • 消耗卡路里：{summary['calories']:.0f} 卡")
        if summary.get('intensity_minutes', 0) > 0:
            lines.append(f"  • 中高强度运动：{summary['intensity_minutes']} 分钟")
    ...
    # 根据睡眠给建议
    if sleep.get('duration_hours', 0) < 6:
        lines.append("  • 睡眠不足，今晚早点休息")
    elif sleep.get('duration_hours', 0) < 7:
        lines.append("  • 睡眠略少，建议保证7-8小时")

    # 根据身体电量给建议
    if body_battery.get('current', 0) < 40:
        lines.append("  • 身体电量较低，避免高强度训练")
    elif body_battery.get('current', 0) >= 70:
...
def generate_evening_report(data):
    ...
    # 身体电量总结
    body_battery = data.get('body_battery', {})
    if body_battery.get('drained', 0) > 0:
        lines.append("🔋 身体电量总结")
        lines.append(f"  • 充电：{body_battery['charged']}%")
        lines.append(f"  • 消耗：{body_battery['drained']}%")
        if body_battery.get('current', 0) > 0:
            lines.append(f"  • 当前剩余：{body_battery['current']}%")
        lines.append("")
    ...
    # 根据身体电量给建议
    if body_battery.get('current', 0) < 30:
